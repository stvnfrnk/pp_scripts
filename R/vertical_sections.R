...
total_vertical <- read_excel("~/Desktop/EGRIP /data/_statistics_total/total_EGRIP.xls") # read_excel was used to load in an .xls file, a different import method has to be used for a different file format

total_horizontal <- read_excel("~/Desktop/EGRIP /data/_statistics_total/total_horizontal.xls")    # import main horizontal data file. change the file path/name of file to the used one.
...
# plot of section means. vertical measurements are plotted  first and horizontal one can be added later
  ...
  ggplot( legend = TRUE) +                                                        # activate GGPLOT to start plotting 
   # geom_vline(xintercept = 0.166666, linetype="solid", color = "gray60") +      # activate to insert a vertical line -> indicates strength of anisotropy
    # geom_vline(xintercept = 0.3333333, linetype="solid", color = "gray60") +  
    #geom_vline(xintercept = 0.666666, linetype="solid", color = "gray60") + 
 #   theme(legend.title=element_blank()) +                                        # get rid of legend title
    ...
    scale_y_reverse(limits = c(2100, 100), breaks=seq(100, 2100, 100) ) +         # define y axis -> y-axis direction is reversed. change numbers for different spacings and ticks
...
filter( Center_depth != 645.38 ) %>%                  # filter out invalid sample 1174_1, only half of the thin section
 # filter(  Center_depth > 900 & Center_depth < 1400) %>%
  ...
  scale_x_continuous(breaks=seq(0, 15, 2.5), limits= c(0, 12.5)) +              # preferences for x-scale, change breaks for x-ticks and limits for start and end of axis
