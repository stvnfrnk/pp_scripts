...
pathlisting <- read.table("pathlisting.txt")

##depth <- stat$topdepth_m
##core_az <- stat$orient_deg

###files <- list.files(path="cAxes_stereo_files")
directories <- pathlisting$V1

for (f in directories) {
  Split <- strsplit(f,"/")
  lastdirectory <- Split[[1]][[length(Split[[1]])]]
  sample <- strsplit(f,"EGRIP")[[1]][2]
  bag <- strsplit(sample,"_")[[1]][1]
  section <- strsplit(sample,"_")[[1]][2]
  
  out1 <- paste("schmidt/png/EGRIP_", bag, "_", section, "_stereo", sep = "")
  png(file=paste(out1,".png",sep=""), width=200, height=200, title = as.character(sample),family="cmr10")
  
#  out2 <- paste(f, "/cAxes/stereo", sep = "")
#  png(file=paste(out2,".png",sep=""), width=200, height=200, title = as.character(sample),family="cmr10")
...
#  pdf(file=out, width=3, height=3, title = as.character(sample),family="CM Roman")
  ...
  data <- read.table(paste(f, "/cAxes/stereo.txt", sep=""))
